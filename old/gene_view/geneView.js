//this list contains patients for which part(s) of the questionnaire are unavailable
var incompletePatients;
...
var patientGenes={};
//This variable is used as counter, when isDone = 3, the script is done
var isDone = 0;
...
function loadEnsemblInfoOfGene(geneId, resultDiv){
	...
	$.get('/api/v2/genes/'+geneId).done(function(geneInfo){
    	...
    	var start = geneInfo['start'];
   	 	var end = geneInfo['stop'];
   	 	var type = geneInfo['gene_type'];
   	 	var description = geneInfo['gene_info'];
   	 	if(description!==''){
       		description+='<br/>'
	    }
   	 	var mimAcc = geneInfo['omim_morbid_accesion'];
   	 	var mimDesc = geneInfo['omim_morbid_description'];
	    ...
	    $(resultDiv).append('Position: '+start+'...'+end+'<br/>');
		console.log(mimAcc, mimDesc);
	    /*if(mimAcc !== '[]'){
	    	$(resultDiv).append('Omim morbid disease(s):<br/>');
	    	$.each(mimAcc, function(i, acc){
	    		$(resultDiv).append('<a href="http://www.omim.org/entry/'+acc+'" target="_blank">'+mimDesc[i]+'</a><br/>');
	    	});
	    };*/
...
function getPatientsInRegion(start, stop, geneId, sortCallback, tableDiv, geneTable){	
	...
	$.get('/api/v2/chromosome6_array?attrs=array_id,ownerUsername,imbalance&q=Start_positie_in_Hg19=le='+
		stop+';Stop_positie_in_Hg19=ge='+start+';Chromosoom==6&num=4000').done(function(patients_with_broken_gene){
		...
		if(patients.length !== 0){
			...
			$.each(patients, function(i, patient){
				...
				$.get('/api/v2/chromosome6_a_c?attrs=id,ownerUsername&q=ownerUsername=='+name).done(function(patient){
					console.log(patient['items'].length);
...
/**This function calls the function that processes the answers of a part of the questionnaire*/
	...
	getChrAnswerData(id, part, processPatientsAnswer, function(){
		...
		if(lastGene === geneId && lastPatient === name){
			...
			isDone+=1;
			//when all parts of the last patient of the last gene are done (and thus isDone = 3), sort the genes and put them in table
			if(isDone ===3){
		...
		//if the last patient of the list is not completely filled in, this patient cannot be processed. 
		}else if(lastGene === geneId && $.inArray(lastPatient, incompletePatients)!==-1){
			//make the gene results table
			if(geneTable!== undefined){
				putGenesPerPatientInTable(geneTable, selectedGenes);
			}
