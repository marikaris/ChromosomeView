    ...
    def parseGeneFile(self):
        ...
        for line in open(self.fileName):
            if line.startswith("Associated"):
                ...
                gene_acc = geneInfo[3]
                gene_desc = geneInfo[4]
                gene_ens = geneInfo[5]
                gene_info = geneInfo[6]
                gene_type = geneInfo[7].strip("\n")
                if not gene_ens in self.genes:
                    gene = self.makeGene(geneName, geneStart, geneStop, gene_acc, gene_desc, gene_ens, gene_info, gene_type)
                    self.addGeneToGeneDict(gene, gene_ens)
                else:
                    gene = self.genes[gene_ens]
                    self.updateGene(gene_acc, gene_desc, geneStart, geneStop, gene)
        print("Gene file parsed")

    def makeGene(self, name, start, stop, mim_accesion, mim_description, ensembl, gene_info, gene_type):
        """function makeGene
        makes a gene object
        ...
        """
        gene = Gene(name, start, stop, mim_accesion, mim_description, ensembl, gene_info, gene_type)
        return gene
